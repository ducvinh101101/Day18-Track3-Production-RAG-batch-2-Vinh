...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < threshold:
            # Nếu similarity thấp hơn threshold -> tạo chunk mới
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    # 1. Gom các paragraph thành parent chunks (mỗi parent ≤ parent_size kí tự)
    current_parent_text = ""
    ...
    # 2. Với mỗi parent, chia thành các child chunks (mỗi child ≤ child_size kí tự)
    for parent in parents:
        ...
        for sent in parent_sentences:
            if len(current_child_text) + len(sent) + 1 > child_size and current_child_text:
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    import re
    metadata = metadata or {}
    # Split text theo header markdown
    ...
    for part in parts:
        ...
        if re.match(r'^#{1,3}\s+', part_strip):
            # Nếu gặp header mới và trước đó có nội dung -> đóng gói chunk trước
