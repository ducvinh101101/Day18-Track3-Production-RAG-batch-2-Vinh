...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    try:
        ...
        try:
            ...
            custom_embeddings = HuggingFaceEmbeddings(model_name=EMBEDDING_MODEL)
        except Exception as emb_e:
            print(f"  ⚠️  Failed to load HuggingFaceEmbeddings: {emb_e}")
        ...
        answer_relevancy_score = float(df["answer_relevancy"].mean()) if "answer_relevancy" in df else 0.0
        context_precision_score = float(df["context_precision"].mean()) if "context_precision" in df else 0.0
        context_recall_score = float(df["context_recall"].mean()) if "context_recall" in df else 0.0
...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    for r in eval_results:
        ...
        diagnosis, suggested_fix = diagnostic_tree.get(worst_metric, ("Unknown issue", "N/A"))
    ...
    analyzed = sorted(analyzed, key=lambda x: x["avg_score"])
